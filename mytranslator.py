...
class Translator(object):
    ...
    def __init__(
        ...
        self.turbo_decoder = turbo_transformers.TransformerDecoder.from_onmt(self.model.decoder)
    ...
    def translate_batch(self, batch, src_vocabs, attn_debug):
        ...
        with torch.no_grad():
            ...
            if show_profile_detail:
                print(f"BeamSearch Time {beam_search_time:0.4f} seconds, translate_batch_with_strategy Time {translate_batch_with_strategy_time: 0.4f} seconds")
            return ret
    ...
    def _decode_and_generate(
        ...
        if show_debug_detail:
            print("debug in _decode_and_generate, memory_bank", memory_bank.size(), memory_bank.type())
            print("debug in _decode_and_generate, decoder_in", decoder_in.size(), decoder_in.type())
            print('debug in _decode_and_generate, memory_lengths', memory_lengths, memory_lengths.type())
            
            print('debug max_relative_positions', self.model.decoder.transformer_layers[0].self_attn.max_relative_positions) # always 0
            print('debug word_vec_size', self.model.decoder.embeddings.word_vec_size)
            print('debug word_padding_idx', self.model.decoder.embeddings.word_padding_idx)
    ...
    def _translate_batch_with_strategy(
        ...
        src, enc_states, memory_bank, src_lengths = self._run_encoder(batch)
        if show_debug_detail:
            print("debug init state src", src.size(), src.type())
            print("debug init state memory_bank", memory_bank.size(), memory_bank.type())
            print("debug init state enc_states", enc_states.size(), enc_states.type())
        self.model.decoder.init_state(src, memory_bank, enc_states)
        self.turbo_decoder.init_state(src, memory_bank, enc_states)
        ...
        if fn_map_state is not None:
            ...
            # fjr turbo
            self.turbo_decoder.map_state(fn_map_state)

        # (3) Begin decoding step by step:
        deocder_and_generate_time = 0.
        for step in range(decode_strategy.max_length):
            ...
            toc = time.perf_counter()
            deocder_and_generate_time += toc - tic
            ...
            if parallel_paths > 1 or any_finished:
                ...
                self.turbo_decoder.map_state(
                    lambda state, dim: state.index_select(dim, select_indices)
                )
            
        if show_profile_detail:
            print(f"deocder_and_generate_time {deocder_and_generate_time:0.4f}")
